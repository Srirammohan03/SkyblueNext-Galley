...
export async function POST(req: Request) {
    ...
    try {
        const result = await prisma.$transaction(async (tx) => {
            // Fetch flight
            const flight = await tx.flightOrder.findUnique({ where: { id: flightId } });
            if (!flight) throw new Error("Flight not found");

            // Determine warehouse location
            const warehouseLoc = await tx.inventoryLocation.findFirst({ where: { type: "WAREHOUSE" } });
            if (!warehouseLoc) throw new Error("Warehouse location missing");

            const requested = Math.round(Number(baseUnits));

            // Calculate available restored (catalog-level) for this tailNumber
            const restoredPool = await tx.restoredItem.findMany({
                where: {
                    flightOrder: { tailNumber: flight.tailNumber, status: "Completed" },
                    returnedQty: { gt: 0 },
                },
